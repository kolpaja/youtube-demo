        ...
        if (months < 12 && months > 0) {
            ...
            if (days >= 7 && days <= 31) {
                ...
                if (days < 7 && days > 0) {
                    ...
                    return `${Math.floor(minutes / 60)} hour${minutes == 60 ? '' : 's'} ago`
                } else {
...
function renderVideoCard(items) {
    ...
    items.forEach((item) => {
        ...
        const minutes = parseInt(moment().diff(moment(`${item.snippet.publishTime}`).format('YYYY-MM-DD'), 'minute'));
        const days = parseInt(moment().diff(moment(`${item.snippet.publishTime}`).format('YYYY-MM-DD'), 'day'))
        const months = parseInt(moment().diff(moment(`${item.snippet.publishTime}`).format('YYYY-MM-DD'), 'month'))

        let videoId = item.id.videoId;
        console.log('videoId: ', videoId);
        searchVideosViewCount(API_key, videoId);
        ...
        <div class="video-history"><span class="views">1M views</span><i class="fas fa-dot icon-dot"></i><span class="posted-date"></span>${PostedTime(minutes, days, months)}</div>
